...
distance_y = [170, 100, 78, 151, 200, 200, 280, 300, 305, 280, 500, 550, 525, 500]
# 城市距离和信息素
INF = 999999999
plant_length = 0 # 无人机总路程 29 # 创建初始化距离矩阵
distance_graph = [[0, 72, 98, 133, 54, 105, 55, 83, 79, 140, 26, 50, 121, 115],  # 车到每个点到其他点的距离
...
plant_path = [
    [0, 20, 76, 91, 38, 89, 52, 83, 79, 117, 26, 50, 84, 115],
...
class Ant( object):
    ...
    def __clean_data(self):
        ...
        for i in range(len(plant1)):
            self.open_table_city[plant1[i]] = False
        self.move_count = 8
    # 选择下一个城市
...
class TSP( object):
    ...
    def new(self, evt=None):
        ...
        for i in range(city_num):
            ...
            # 生成节点椭圆，半径为self.__r
            node = self.canvas.create_oval(x - 15,
                                           y - 15, x + 15, y + 15,
                                        fill="#7FFF00", # 填充绿色
                                        outline="#000000", # 轮廓红色
                                        tags="node",
            ...
            # 显示坐标
            self.canvas.create_text(x, y, # 使用create_text方法在坐标（302，77）处绘制文字
                                    text=i + 1,  # 所绘制文字的内容
    ...
    def line(self, order, plant_length):
        ...
        time_plant = 0
        time_plant = time_plant + ((plant_graph[order_3[0]][order_3[1]] + plant_graph[order_3[1]][ order_3[2]]) / 75)
        ...
        time_plant = time_plant + ((plant_graph[order_3[4]][order_3[5]] + plant_graph[order_3[5]][
            order_3[6]] +
                                    plant_graph[order_3[6]][order_3[7]]) / 75)
        all_time = all_time + max(time_car, time_plant)

        all_time = all_time + (car_graph[1][4]+car_graph[4][5]) / 50

        time_car = (car_graph[order_4[5]][order_4[6]]) / 50
        time_plant = 0
        time_plant = time_plant + ((plant_graph[order_3[8]][order_3[9]] + plant_graph[order_3[9]][
        order_3[10]]) / 75)
        all_time = all_time + max(time_car, time_plant)
...
a = 1.1
D = 87.5
xq = [12, 90, 24, 15, 70, 18, 150, 50, 30, 168, 36, 44, 42, 13]  # 给个地点需求量需求
hc = []  # 货车停靠点
fj = []  # 可能的飞机停靠点
                ...
                for i1 in range(k):
                    ...
                    while j1 < n:
                        if s[i1][j1] == i: # 如果这个被处理过的点离当前的两个汽车停靠点更近，则将点移到这里
                            if plant_path[d_tkd[i1][0]][i] + plant_path[d_tkd[i1][1]][i] >plant_path[p1][i] + \
                                plant_path[p2][i]:
                                del s[i1][j1]
                                n = n - 1
                                s[k].append(i)
...
for i in range( len(car)):
    for j in range( len(plant)):
        X = distance_graph[car[(i - 1) % 7]][plant[j]] + distance_graph[car[(i + 1) % 7]][plant[j
        ]] - distance_graph[car[(i - 1) % 7]][car[i]] - \
        distance_graph[car[(i + 1) % 7]][car[i]]
    if X<max1:
        x = car[i]
        y = plant[j]
